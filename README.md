...
- `npm run phase:autopilot -- --message=\"chore: ...\" --cluster-release` — include release evidence/handoff finalization in the same closeout run (`--release-from`, `--release-to`, `--release-limit`, `--release-skip-watch`)
- `npm run phase:autopilot -- --message=\"chore: ...\" --cluster-all` — preset for one-go closeout + release finalization + next-branch bootstrap (`--cluster-release + --start-next`)
...
- `npm run release:notes -- --from=<ref> --to=<ref>` — generate markdown release notes grouped by commit type (or latest commits via `--limit=<n>`)
- `npm run release:bundle -- --from=<ref> --to=<ref>` — generate a timestamped release bundle under `logs/` (release notes + ops snapshot + incident template)
- `npm run release:closeout -- --from=<ref> --to=<ref>` — run release bundle capture and then `ops:watch` in one command for release handoff confirmation
...
- `npm run release:go-live` — one command: `git fetch`, `branch:check`, `readiness:report -- --release`, `ops:watch`, timestamped `logs/release-go-live-*.md`, annotated release tag `vYYYY.MM.DD-HHMM` (+ push to `origin` when GO LIVE: YES), optional Slack post via `SLACK_WEBHOOK_URL`, plus a Slack/email broadcast block on stdout; use `--no-tag` / `--no-slack` to skip tagging or webhook
